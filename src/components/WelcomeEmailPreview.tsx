...
export function WelcomeEmailPreview({ 
  ...
  useEffect(() => {
    const fetchTemplate = async () => {
      try {
        setIsLoading(true);
        const emailTemplate = await getEmailTemplateByName('Welcome Email');
        ...
        console.error('Error fetching welcome email template:', err);
    ...
    fetchTemplate();
  }, [logoUrl, companyName, primaryColor, userName, userEmail, userRole, organization]);
  ...
  return (
    <div className="border rounded-md overflow-hidden">
      <div className="bg-gray-50 px-4 py-2 border-b">
        <h4 className="text-sm font-medium text-gray-700">Welcome Email Preview</h4>
      </div>
      <div className="p-4 bg-white">
        <div className="mb-2">
          <span className="text-sm font-medium text-gray-700">Subject: </span>
          <span className="text-sm text-gray-900">Welcome to {companyName}</span>
        </div>
